...
trait BlueprintTrails extends Trails{
  ...
  private def ontoE(edgeName: String, dir: Direction): Traverser[Edge] =
    e => ts => ts match {
      case ((path@((head: Vertex) :: rest), visitedPaths), state) =>
        head.getEdges(dir, edgeName).toStream.map { edge => ((((edge :: path), visitedPaths), state), edge) }
    }
  ...
  private def ontoV(dir: Direction): Traverser[Vertex] =
    _ => ts => ts match {
      case ((path@((head: Edge) :: rest), visitedPaths), state) => {
        val v = head.getVertex(dir)
        Stream(((((v :: path), visitedPaths),state), v))
      }
    }
